...
bplot1 <- ggplot(peaks, aes(x = peaks$Experiment, y = peaks$Peak_count)) +
                ...
                labs(title = "Peak counts within experiments",
                     x = "Experiment", y = "Peak counts")
...
calculate_peaks <- function(filename) {
    ...
    file <- import(paste0("../BigBed/", name)) %>% filter(seqnames %in% chr_abr)
...
for (i in 1:length(peak_sequences)) {
    #filename_short <- samples$Graph_names[samples$Filename == peak_sequences[i]]
    filename <- peak_sequences[i]
    seq <- readDNAStringSet(paste0("../FASTA/", filename))
    #percentage <- as.numeric(find_motif_hits(seq)) / 
    #              as.numeric(calculate_peaks(filename))
    output <- c(filename, find_motif_hits(seq), calculate_peaks(filename))

    tbx5_hits <- rbind(tbx5_hits, output)
    #print(paste0("Mėginys turi ", calculate_peaks(filename), " ir ",
    #             find_motif_hits(seq), " tbx5 motyvo atitikimų."))
}
