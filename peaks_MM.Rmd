...
png(file = "./peak_counts_by_chromosome.png",
...
png(file = "Latex/Figures/peak_overlaps_between_samples.png", width = 600)
overlaps
dev.off()
```
...
calculate_peaks <- function(filename) {
    ...
    file <- import(paste0("../BigBed/", name)) %>%
                    filter(seqnames %in% chr_abr)
...
# tbx5_motif <- data.frame(matrix(nrow = 0, ncol = length(columns))) 
 
# colnames(tbx5_motif) <- columns
...
#     tbx5_motif[nrow(tbx5_motif) + 1, ] <- data_row
# }

# write.csv(tbx5_motif, "tbx5_motif_data.csv")
# # THE END OF THE COMMENT

motif_data <- read.csv(file = "tbx5_motif_data.csv")
...
tf_percent <- ggplot(melted_df, aes(x = Sample, y = value, fill = variable,
                ...
                scale_fill_manual(values = c("#ebc28e", "#d18558"),
...
png(file = "Latex/Figures/tf_hit_percentage.png", width = 500)
tf_percent 
dev.off()
```
...
```{r}
identified_motifs <- read.csv(file = "identified_motifs.csv")
identified_motifs

library(ggplot2)


barplot(identified_motifs$Motif_count, names.arg = identified_motifs$Sample)


identified_motifs$Sample <- factor(identified_motifs$Sample, levels = identified_motifs$Sample)
ggplot(identified_motifs, aes(x = Sample, y = Motif_count)) +
